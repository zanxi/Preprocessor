...
void OS::more(const string& filename)
{
  ...
  while (true) {
    ...
    for (i = currLine; i < allLines.size() && i - currLine < maxLines; i++) {
      ...
      logger::WriteLog(QString::fromStdString(allLines[i]));
    ...
    logger::WriteLog(QString::fromStdString(to_string(percent))+"% *** Press <space> to continue, 'b' to go back, or any key to exit ***");
